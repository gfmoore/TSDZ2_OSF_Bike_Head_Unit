...
const MainNavigator = () => {
  ...
  const [motor_Voltage,                             setMotor_Voltage]                                 = useState(999)
  ...
  const loadStateFromAsyncStorage = async () => {
    //#region get data from async storage and put it into state
    try {
      //#region set state from async storage
      ...
      setTorque_ADC_Offset(await AsyncStorage.getItem('torque_ADC_Step'))
      ...
      setWalkAssist(await AsyncStorage.getItem('walkAssist'))
  ...
  //#region helper functions for async storage
  ...
  const getAllkeys = async () => {
    ...
    try {
      ...
      console.log(`GM Error retrieving all keys: ${e}`)
    }
  }
  ...
  useEffect(async () => {
    ...
    try {
      ...
      if (keys.length === 0) {
        //saveStateToAsyncStorage()  //next time we have async storage
      }
      else {
